...
def _segm_mobilenet_SA(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    if output_stride == 8:
        aspp_dilate = [12, 24, 36]
    else:
        aspp_dilate = [6, 12, 18]
    ...
    attention=Self_Attn(inplanes,'relu')
...
def deeplabv3plus_mobilenetSA(num_classes=21, output_stride=8, pretrained_backbone=True):
